...
# Remove the IP from the load balance pool
url = URI.parse("https://api2.dynect.net/REST/LoadBalancePoolEntry/#{ZONE}/#{FQDN}/#{IPADDR}") 
resp, data = http.delete(url.path, headers)

print 'Delete LoadBalancePoolEntry Response: ', data, '\n'; 
